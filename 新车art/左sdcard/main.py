...
uart = UART(1, baudrate=115200)     #老车左为uart2，若换sd卡记得修改
#检查是否都初始化为False
do_classify=False
do_line = False
rect_flag = False
...
while(True):
    ...
    for r in img.find_rects(threshold = threshold,quality = rect_select):             # 在图像中搜索矩形
        ...
        blob_angle = get_point_point_angle(p2, p3)
        x_error = img_x - pic_x
        y_error = img_y - pic_y
        if (x_error < 0):
            x_flag = 0  # 正负标志位
        ...
        if (y_error < 0):
            ...
            y_flag = 1
        #print("error:" + str(x_error) + "," + str(y_error)+","+str(blob_angle))
        uart_array1 = bytearray([0XF1,0xF2,x_error,y_error,x_flag,y_flag,blob_angle,0XF3])
        ...
        if do_classify:
            ...
            if model_select:
                ...
                uart_array2 = bytearray([0XAF,0XAF])#没有识别出任何东西

                if result_index== '1':#1
                    uart_array2 = bytearray([0XAF,0xB0])
                if result_index == '2':#2
                    uart_array2 = bytearray([0XAF,0xB1])
                if result_index == '3':#3
                    uart_array2 = bytearray([0XAF,0xB2])

                if result_index == 'A':#A
                    uart_array2 = bytearray([0XAF,0xC0])
                if result_index == 'B':#B
                    uart_array2 = bytearray([0XAF,0xC1])
                if result_index == 'C':#C
                    uart_array2 = bytearray([0XAF,0xC2])

                if result_index == 'D':#D
                    uart_array2 = bytearray([0XAF,0xC3])
                if result_index == 'E':#E
                    uart_array2 = bytearray([0XAF,0xC4])
                if result_index == 'F':#F
                    uart_array2 = bytearray([0XAF,0xC5])

                if result_index == 'G':#G
                    uart_array2 = bytearray([0XAF,0xC6])
                if result_index == 'H':#H
                    uart_array2 = bytearray([0XAF,0xC7])
                if result_index == 'I':#I
                    uart_array2 = bytearray([0XAF,0xC8])

                if result_index == 'J':#J
                    uart_array2 = bytearray([0XAF,0xC9])
                if result_index == 'K':#K
                    uart_array2 = bytearray([0XAF,0xCA])
                if result_index == 'L':#L
                    uart_array2 = bytearray([0XAF,0xCB])

                if result_index == 'M':#M
                    uart_array2 = bytearray([0XAF,0xCC])
                if result_index == 'N':#N
                    uart_array2 = bytearray([0XAF,0xCD])
                if result_index == 'O':#O
                    uart_array2 = bytearray([0XAF,0xCE])
                #print("1")
                ...
                if result_index== 'firearms':#枪
                    uart_array2 = bytearray([0XAF,0xA0])
                if result_index == 'explosive':#炸弹
                    uart_array2 = bytearray([0XAF,0xA1])
                if result_index == 'dagger':#匕首
                    uart_array2 = bytearray([0XAF,0xA2])
                #上1
                if result_index == 'spontoon':#警棍
                    uart_array2 = bytearray([0XAF,0xA3])
                if result_index == 'fire_axe':#消防斧
                    uart_array2 = bytearray([0XAF,0xA4])
                if result_index == 'first_aid_kit':#急救包
                    uart_array2 = bytearray([0XAF,0xA5])
                #下2
                if result_index == 'flashlight':#手电筒
                    uart_array2 = bytearray([0XAF,0xA6])
                if result_index == 'intercom':#bb机
                    uart_array2 = bytearray([0XAF,0xA7])
                if result_index == 'bulletproof_vest':#防弹衣
                    uart_array2 = bytearray([0XAF,0xA8])
                #左3
                if result_index == 'telescope':#望远镜
                    uart_array2 = bytearray([0XAF,0xA9])
                if result_index == 'helmet':#头盔
                    uart_array2 = bytearray([0XAF,0xAA])
                if result_index == 'fire_engine':#救火车
                    uart_array2 = bytearray([0XAF,0xAB])
                #右4
                if result_index == 'ambulance':#救护车
                    uart_array2 = bytearray([0XAF,0xAC])
                if result_index == 'armoured_car':#装甲车
                    uart_array2 = bytearray([0XAF,0xAD])
                if result_index == 'motorcycle':#摩托车
                    uart_array2 = bytearray([0XAF,0xAE])
                #print("2")
                #print("%s: %f" % (result_index, m))
                uart.write(uart_array2)
                do_classify=False
